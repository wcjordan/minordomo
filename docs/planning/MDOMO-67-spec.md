...
- Recipient: `${DOMAIN_ROOT}@gmail.com` (derived from `ROOT_DOMAIN` global env var, same convention as `JENKINS_USERNAME`)
...
- `shared/setup-env.sh` — add `export NOTIFICATION_EMAIL="${DOMAIN_ROOT}@gmail.com"` after the `DOMAIN_ROOT` derivation line
- `docs/setup/aws-ses-setup.md` — setup instructions for the human:
  ...
  - How to verify the sender email address in SES (both sender and recipient are the same `${DOMAIN_ROOT}@gmail.com` address, so one verification covers both)
...
- When `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` are set to valid SES credentials, script sends email (manual integration test after credential setup)
...
import json, sys
try:
    d = json.loads(open('/tmp/prompt-output.txt').read().strip().splitlines()[-1])
...
Notes:
- Parse only the last non-empty line (the run log JSON is the final line of `prompt-output.txt`)
...
- When `aws-ses-access-key-id` and `aws-ses-secret-access-key` credentials exist in Jenkins and the build fails, an email is sent to `${DOMAIN_ROOT}@gmail.com`
